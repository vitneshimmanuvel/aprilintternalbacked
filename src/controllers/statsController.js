...
const getTodayStats = async (req, res, next) => {
  try {
    ...
    if (from_date && to_date) {
      dateFilterSql = "DATE(created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata') BETWEEN $1 AND $2";
      params = [from_date, to_date];
    } else if (from_date) {
      dateFilterSql = "DATE(created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata') = $1";
      params = [from_date];
    }
    ...
    const newLeads = await pool.query(`
      ...
      WHERE ${dateFilterSql.replace(/created_at/g, 'l.created_at')}
    ...
    const allActions = await pool.query(`
      ...
      WHERE ${dateFilterSql.replace(/created_at/g, 'h.created_at')}
    ...
    const visitsToday = await pool.query(`
      ...
      WHERE ${dateFilterSql.replace(/created_at/g, 'v.created_at')}
